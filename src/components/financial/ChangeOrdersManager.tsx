...
import { Plus, FileText, Clock, CheckCircle, XCircle, Eye, Edit } from "lucide-react";
...
export default function ChangeOrdersManager({ projectId }: ChangeOrdersManagerProps) {
    ...
    const mockOrders: ChangeOrder[] = [
      {
        ...
        description: "Install 5 additional culverts at chainage 15+500 to 16+200 due to increased water flow",
        reason: "Heavy rainfall analysis shows need for enhanced drainage capacity",
        costImpact: 2500000,
        timeImpact: 14,
        ...
        implementation: {
          ...
          actualCost: 2650000,
          notes: "Completed on schedule with minor cost overrun due to equipment mobilization"
        }
      },
      {
        id: "co2",
        ...
        description: "Upgrade pavement thickness from 150mm to 200mm for heavy vehicle sections",
        reason: "Traffic study indicates higher heavy vehicle loads than originally designed",
        costImpact: 4200000,
        timeImpact: 21,
      ...
      },
      {
        ...
        description: "Additional sediment control measures and wildlife crossing",
        reason: "Environmental compliance requirements updated during construction",
        costImpact: 1800000,
        timeImpact: 10,
      ...
      },
      {
        ...
        description: "Modify bridge foundation design due to unexpected soil conditions",
        reason: "Geotechnical investigation revealed soft clay layer requiring deeper foundations",
        costImpact: 5500000,
        timeImpact: 30,
      ...
      },
      {
        ...
        description: "Extend safety barriers by 2km due to updated safety standards",
        reason: "PNG safety standards updated requiring additional barrier protection",
        costImpact: 800000,
        timeImpact: 7,
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "submitted": case "under-review": return <Clock className="h-4 w-4" />;
  ...
  const updateOrderStatus = (orderId: string, newStatus: ChangeOrder['status']) => {
    setChangeOrders(prev => prev.map(order =>
      order.id === orderId
        ? {
            ...order,
            status: newStatus,
  ...
  const totalCostImpact = changeOrders
    .filter(order => order.status === 'approved' || order.status === 'implemented')
  ...
  const pendingOrders = changeOrders.filter(order =>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-bold text-gray-900">Change Orders Management</h3>
          <p className="text-gray-600">Track and manage project scope changes</p>
        </div>
      ...
      {showAddForm && (
        <Card className="border-blue-200 bg-blue-50">
          <CardHeader>
            <CardTitle className="text-blue-800">Create New Change Order</CardTitle>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setNewOrder(prev => ({...prev, costImpact: Number(e.target.value)}))}
                />
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setNewOrder(prev => ({...prev, timeImpact: Number(e.target.value)}))}
                />
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setNewOrder(prev => ({...prev, requestedBy: e.target.value}))}
                />
              </div>
            </div>

            <div className="flex gap-2">
              <Button onClick={addChangeOrder} className="bg-blue-600 hover:bg-blue-700">
                Create Change Order
              </Button>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>All project change orders and their current status</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {changeOrders.map((order) => (
                <TableRow key={order.id}>
                  ...
                  <TableCell>
                    <span className={`font-semibold ${order.costImpact > 0 ? 'text-red-600' : 'text-green-600'}`}>
                      {order.costImpact > 0 ? '+' : ''}{formatCurrency(order.costImpact)}
                    </span>
                  </TableCell>
                  <TableCell>
                    <span className={`font-semibold ${order.timeImpact > 0 ? 'text-red-600' : 'text-green-600'}`}>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {['draft', 'submitted', 'under-review', 'approved', 'rejected', 'implemented'].map(status => {
              const count = changeOrders.filter(order => order.status === status).length;
              return (
